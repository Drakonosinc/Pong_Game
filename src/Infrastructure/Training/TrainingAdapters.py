...
class GeneticTrainer(ITrainer):
    def train(self, game):
        print("[GeneticTrainer] Iniciando algoritmo genético...")
...
class QLearningTrainer(ITrainer):
    def train(self, game):
        print("[QLearningTrainer] Iniciando Q-Learning...")
        cfg = game.config.config_AI
        nn_cfg = cfg.get("nn", {"hidden_layers": 2, "neurons_per_layer": 6})
        arch = [nn_cfg.get("neurons_per_layer", 6)] * nn_cfg.get("hidden_layers", 2)
        type_model_str = next(k for k, v in cfg["type_model"].items() if v)
        ...
        best_model = q_learning_algorithm(
            ...
            hidden_sizes=arch)
        if cfg["model_save"] and best_model is not None: save_qlearning_model(best_model, torch.optim.Adam(best_model.parameters(), lr=0.001), game.model_path)
        return best_model