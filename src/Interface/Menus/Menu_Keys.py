...
class KeysMenu(BaseMenu):
    ...
    def setup_buttons(self):
        ...
        self.buttons['back'] = factory_f5.create_PolygonButton({"position": ((50, 350), (50, 380), (25, 365)),"position2": ((50, 340), (50, 390), (10, 365)),"command1": lambda: self.change_mains({"main": GameState.OPTIONS})})
        ...
        self.buttons['default'] = factory_f5.create_TextButton({"text": "Default config","position": (self.WIDTH/2+160, self.HEIGHT/2+160),"command1": lambda: (self.config.config(keys=True),self.change_mains({"main": GameState.KEYS, "command": self.setup_buttons}))})
    def render(self):
