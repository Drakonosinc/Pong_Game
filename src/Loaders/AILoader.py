...
class AILoader:
    def __init__(self, context_or_config):
        if hasattr(context_or_config, "base_dir") and hasattr(context_or_config, "config_AI"): self.config = context_or_config
    ...
    def load_model_result(self) -> AIModelLoadResult:
        ...
        try:
            ...
            if not os.path.exists(model_path): return AIModelLoadResult(model=None, model_found=False, error_message=None)
